...
class TestOffices(Base):
    ...
    def test_create_office(self):
        """ Tests that office successfully created """

        res = self.client.post('/api/v1/offices', json=self.new_office)
        data = res.get_json()

        self.assertEqual(data['status'], 201)
        self.assertEqual(data['message'], 'Office successfully created')
    ...
    def test_get_all_offices(self):
        ...
        self.assertEqual(data['message'], 'Request successfully sent')
    ...
    def test_get_sigle_office(self):
        """ Tests when get request made to /offices/<int:id> """
    ...
    def test_get_single_office_id_not_found(self):
        """ Tests request made with id that doesn't exist """
    ...
    def test_delete_office(self):
        ...
        self.assertEqual(data['status'], 200)
        self.assertEqual(data['message'], 'Governor successfully deleted')
        self.assertEqual(len(data['data']), 1)
        self.assertEqual(data['data'][0]['id'], 1)
        self.assertEqual(res.status_code, 200)

    def test_delete_office_id_not_found(self):
        """ Tests DELETE request made with id that doesn't exist """
